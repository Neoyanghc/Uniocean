...
class TokenEmbedding_new(nn.Module):
    ...
    def forward(self, x):
        ...
        B, C, H, W = x.shape
        ## 分patch N为patch数目
        x = x.reshape(B,C,H//P,P,W//P,P).permute(0, 1, 2, 4, 3, 5).reshape(B,C,-1, P*P)   ## (B, C, H, W)->(B, C, N, P*P)
        _, _, N, _ =x.shape
...
class TimeFeatureEmbedding_new(nn.Module):
    ...
    def forward(self, x, scale=1):
        ...
        concat_tensor.to(x.device)
        x = torch.cat((x, concat_tensor), 2)
        return self.embed(x)

class DataEmbedding(nn.Module):
    def __init__(self, c_in, d_model,channel, embed_type='fixed', freq='h', dropout=0.1):
        super(DataEmbedding, self).__init__()
        
        # self.space_embedding=SpaceEmbedding(c_in=channel, d_model=d_model)
...
class DataEmbedding_scale(nn.Module):
    ...
    def forward(self, x, x_mark, scale, first_scale, label_len):
        ...
        tembed = self.temporal_embedding(x_mark)
...
class DataEmbedding_Spacialpatch_(nn.Module):
    ...
    def forward(self, x, x_mark):
        ...
        x = self.value_embedding(x) + self.patch_embedding(x) + self.position_embedding(x) + self.temporal_embedding(x_mark)
